...
# Step-1 We initialize the Experience Replay Memory Error class
class ReplayBuffer(object):
    ...
    def sample(self, batch_size):
        ...
        return np.array(batch_states), np.array(batch_next_states), np.array(batch_orientation), np.array(
            batch_next_orientation), np.array(batch_actions), np.array(batch_rewards).reshape(-1, 1), np.array(
            batch_dones).reshape(-1, 1)
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, max_action, latent_dim):
        ...
        self.convolution_actor_module = torch.nn.ModuleList([
    ...
    def forward(self, x, o):
        ...
        for layer in self.convolution_actor_module:
            # print("input:", x.size())
            # print(layer)
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim, latent_dim):
        ...
        self.convolution_critic1_module = torch.nn.ModuleList([  ## input size:[28, 28]
            torch.nn.Conv2d(1, 8, 3),
            torch.nn.ReLU(),
            torch.nn.BatchNorm2d(8),
        ...
        self.linear_critic1_module = torch.nn.ModuleList([
        ...
        self.convolution_critic2_module = torch.nn.ModuleList([
            torch.nn.Conv2d(1, 8, 3),
            torch.nn.ReLU(),
            torch.nn.BatchNorm2d(8),
        ...
        self.linear_critic2_module = torch.nn.ModuleList([
            torch.nn.Linear(latent_dim + 2 + action_dim, 16),
            torch.nn.ReLU(),
            torch.nn.Linear(16, 8),
            torch.nn.ReLU(),
    ...
    def forward(self, x, o, u):
        x1 = x
        for layer in self.convolution_critic1_module:
            x1 = layer(x1)
        counter = 0
        for layer in self.linear_critic1_module:
            counter += 1
            if counter == 1:
        ...
        x2 = x
        for layer in self.convolution_critic2_module:
            x2 = layer(x2)
        counter = 0
        for layer in self.linear_critic2_module:
            counter += 1
            if counter == 1:
...
class TD3(object):
    ...
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005, policy_noise=0.2,
        ...
        for it in range(iterations):
            ...
            batch_states, batch_next_states, batch_orientation, batch_next_orientation, batch_actions, batch_rewards, batch_dones = replay_buffer.sample(
                batch_size)
    ...
    def save(self, directory, filename):
        ...
        torch.save(self.actor.state_dict(), actor_path)  # '%s\\%s_actor.pth' % (directory, filename))
        torch.save(self.critic.state_dict(), critic_path)  # '%s\\%s_critic.pth' % (directory, filename))

    # Making a load method to load a pre-trained model
    def load(self, directory, filename):
        actor_path = "{}\\{}_actor.pth".format(directory, filename)
        critic_path = "{}\\{}_critic.pth".format(directory, filename)
        self.actor.load_state_dict(torch.load(actor_path))  # '%s\\%s_actor.pth' % (directory, filename)))
        self.critic.load_state_dict(torch.load(critic_path))  # '%s\\%s_critic.pth' % (directory, filename)))
